...
// List - جلب جميع نقاط العمل مع الموظفين العاملين حالياً مع pagination
func (h *WorksiteHandler) List(c *gin.Context) {
	...
	var total int64
	err := h.DB.QueryRow(`SELECT COUNT(*) FROM worksites WHERE is_deleted = FALSE`).Scan(&total)
	if err != nil {
	...
	for rows.Next() {
		...
		worksiteIDs = append(worksiteIDs, w.ID)
	}

	// حل N+1: جلب الموظفين العاملين حالياً في كل نقطة عمل باستخدام استعلام واحد
	if len(worksiteIDs) > 0 {
		// بناء استعلام ديناميكي لجلب الموظفين العاملين في جميع نقاط العمل
		workingEmployeesMap := make(map[string][]gin.H)

		// استخدام IN clause لجلب الموظفين العاملين في كل نقطة عمل
		placeholders := make([]string, len(worksiteIDs))
		args := make([]interface{}, len(worksiteIDs))
		for i, id := range worksiteIDs {
			placeholders[i] = fmt.Sprintf("$%d", i+1)
			args[i] = id
		}

		query := fmt.Sprintf(`
			SELECT DISTINCT a.worksite_id, u.id, u.full_name, a.id as attendance_id
			FROM users u
			JOIN attendance a ON u.id = a.user_id
			WHERE a.status = 'in_progress'
			AND a.worksite_id IS NOT NULL
			AND u.role = 'employee'
			AND a.worksite_id IN (%s)
		`, strings.Join(placeholders, ","))

		workingRows, err := h.DB.Query(query, args...)

		if err == nil {
			for workingRows.Next() {
				var worksiteID, id, name, attendanceID string
				if err := workingRows.Scan(&worksiteID, &id, &name, &attendanceID); err == nil {
					workingEmployeesMap[worksiteID] = append(workingEmployeesMap[worksiteID], gin.H{
						"id":            id,
						"name":          name,
			...
			// إضافة الموظفين العاملين لكل نقطة عمل
			for i, ws := range worksites {
				if ws["id"] == "unassigned" {
					continue
				}
				if employees, exists := workingEmployeesMap[ws["id"].(string)]; exists {
					worksites[i]["working_employees"] = employees
				} else {
					worksites[i]["working_employees"] = []gin.H{}
				}
			}
		}
	}

	// جلب الموظفين العاملين بدون نقطة عمل
	...
	log.Printf("✅ تم جلب %d نقطة عمل (صفحة %d من %d)", len(worksites), page, totalPages)
